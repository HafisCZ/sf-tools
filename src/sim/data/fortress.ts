...
export type FortressValues = {
  ...
  MageCount: number
  MageLevel: number
  FortificationsLevel: number
}

export const FORTRESS_WARRIOR_MAP: Record<number, FortressUnit> = {
...
export const FORTRESS_ARCHER_MAP: Record<number, FortressUnit> = {
...
export const FORTRESS_MAGE_MAP: Record<number, FortressUnit> = {
...
export const FORTRESS_WALL_MAP: Record<number, FortressUnit> = {
...
function createData(tier: number, source: Record<number, FortressUnit>, isWall = false) {
  ...
  return {
    ...
    Class: classId,
    NoBaseDamage: true,
    Armor: level * CONFIG.fromID(classId).MaximumDamageReduction,
...
function createFortification(level: number) {
  return level > 0 ? [createData(level, FORTRESS_WALL_MAP, true)] : []
}

function createUnits(count: number, level: number, source: Record<number, FortressUnit>) {
...
export function createFortressBattle(values: FortressValues) {
  return {
    ...
    target: [...createFortification(values.FortificationsLevel), ...createUnits(values.ArcherCount, values.ArcherLevel, FORTRESS_ARCHER_MAP), ...createUnits(values.MageCount, values.MageLevel, FORTRESS_MAGE_MAP)]
  }
}
