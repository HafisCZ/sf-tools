...
export type Monster = Record<MainAttribute, { Total: number }> & {
  NoBaseDamage: true
  NoGladiator: true
  Level: number
...
export class MonsterGenerator {
  static readonly MONSTER_NORMAL: unique symbol = Symbol()
  static readonly MONSTER_RAID: unique symbol = Symbol()

  static #MULTIPLIERS: Record<symbol, { Default: number; Health: number }> = {
    [this.MONSTER_NORMAL]: {
      Default: 1,
      Health: 1
    },
  ...
  static #DELTA: Record<symbol, MonsterDelta[]> = {
    [this.MONSTER_NORMAL]: [
      {
        range: [10, 149],
  ...
  // An unsupported level throws like the legacy code did
  static create(monsterType: symbol, monsterLevel: number, monsterClass: CharacterClass, monsterRuneType = 0, monsterRuneValue = 0) {
    const base = this.#DELTA[monsterType === this.MONSTER_RAID ? this.MONSTER_NORMAL : monsterType].find((entry) => monsterLevel >= entry.range[0] && monsterLevel <= entry.range[1]) as MonsterDelta

    const delta = monsterLevel - base.range[0]

    const [resetMain, resetSide, resetCon, resetLuck, resetHealth, resetMin, resetMax, resetArmor] = base.reset

    const [deltaMain, deltaSide, deltaCon, deltaLuck, deltaHealth, deltaMin, deltaMax, deltaArmor] = base.delta
