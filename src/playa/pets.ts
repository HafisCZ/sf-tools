...
type PetDateGenerator = () => Generator<Date>

export type Pet = {
  location: number
  next: [Date, Date] | undefined
  time: 'any' | 'day' | 'night' | 'witch'
  condition: true | ((player: PlayerModel) => unknown)
}
...
// Time ranges
const PetTime: Record<Pet['time'], PetTimeRange[]> = {
  any: [{ start: [0, 0, 0], end: [23, 59, 59] }],
  day: [{ start: [6, 0, 0], end: [17, 59, 59] }],
  night: [
    { start: [0, 0, 0], end: [5, 59, 59] },
    { start: [18, 0, 0], end: [23, 59, 59] }
  ],
  witch: [{ start: [0, 0, 0], end: [0, 59, 59] }]
}
...
const PetDateUtils = {
  weekday: function (day: number): PetDateGenerator {
    return function* () {
      const date = new Date()
      if (date.getDay() === day) {
        yield date
  ...
  season: function (...months: number[]): PetDateGenerator {
    return function* () {
      const date = new Date()
  ...
  month: function (month: number): PetDateGenerator {
    return function* () {
      const date = new Date()
  ...
  day: function (day: number, month: number): PetDateGenerator {
    return function* () {
      const date = new Date()
  ...
  weekend: function (type: number | null = null, date = new Date()): PetDateGenerator {
    return function* () {
  ...
  easterDay: function (year: number) {
  ...
  whitsunDay: function (year: number) {
    const b = Math.floor(year / 100)
    const h = (19 * (year % 19) + b - Math.floor(b / 4) - Math.floor((b - Math.floor((b + 8) / 25) + 1) / 3) + 15) % 30
    const l = (32 + 2 * (b % 4) + 2 * Math.floor((year % 100) / 4) - h - ((year % 100) % 4)) % 7
    const m = Math.floor(((year % 19) + 11 * h + 22 * l) / 451)
...
function isPetAvailable(hours: PetTimeRange[], generator: PetDateGenerator): [Date, Date] | undefined {
...
export const PetData: Pet[] = [
  ...
  },
  {
    ...
    next: isPetAvailable(PetTime.any, PetDate.any),
    time: 'any',
    condition: (player) => player.Dungeons?.Shadow?.[12] === 10
  },
  {
    location: PetLocation.SprawlingJungle,
    next: isPetAvailable(PetTime.any, PetDate.any),
    time: 'any',
    condition: (player) => player.Pets?.Dungeons?.[0] === 20
  },
  {
  ...
  },
  {
    ...
    time: 'any',
    condition: (player) => {
      const group = player.Group?.Group
  ...
  },
  {
    ...
    next: isPetAvailable(PetTime.any, PetDate.any),
    time: 'any',
    condition: (player) => player.Dungeons?.Tower === 100
  },
  {
    location: PetLocation.PlainsOfOzkorr,
    next: isPetAvailable(PetTime.any, PetDate.any),
    time: 'any',
    condition: (player) => player.Pets?.Dungeons?.[1] === 20
  },
  {
  ...
  },
  {
    ...
    condition: (player) => {
      ...
      return fortress && (fortress.Rank as number) > 0 && ((fortress.Rank as number) <= 100 || (fortress.Honor as number) >= 2500)
    }
  },
  {
  ...
  },
  {
    ...
    next: isPetAvailable(PetTime.any, PetDate.any),
    time: 'any',
    condition: (player) => player.Pets?.Dungeons?.[2] === 20
  },
  {
    location: PetLocation.MoldyForest,
    next: isPetAvailable(PetTime.day, PetDate.any),
  ...
  },
  {
    ...
    next: isPetAvailable(PetTime.any, PetDate.any),
    time: 'any',
    condition: (player) => player.Dungeons?.Player === 50
  },
  {
    location: PetLocation.Gnarogrim,
    next: isPetAvailable(PetTime.any, PetDate.any),
    time: 'any',
    condition: (player) => player.Pets?.Dungeons?.[3] === 20
  },
  {
  ...
  },
  {
    ...
    condition: (player) => {
      ...
      return pets && (pets.Rank as number) > 0 && ((pets.Rank as number) <= 100 || (pets.Honor as number) >= 4000)
    }
  },
  {
  ...
  },
  {
    ...
    condition: (player) => {
      ...
      return toilet && (toilet.Aura as number) >= 50
    }
  },
  {
    location: PetLocation.MoldyForest,
    next: isPetAvailable(PetTime.any, PetDate.any),
    time: 'any',
    condition: (player) => player.Dungeons?.Normal?.[24] === 10
  },
  {
    location: PetLocation.BlackWaterSwamp,
    next: isPetAvailable(PetTime.any, PetDate.any),
    time: 'any',
    condition: (player) => player.Pets?.Dungeons?.[4] === 20
  }
]
