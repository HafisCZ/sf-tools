export type ChangelogRelease = string[] | Record<string, string[]>
...
export const CHANGELOG: Record<string, ChangelogRelease> = {
  ...
  'v7.4811': {
    ...
    'Hellevator Simulator': ['Added support for Snacks / Treats'],
    'Raid Simulator': ['Added first 50 normal raids', 'Added hellevator raids'],
    'Dungeon Simulator': ['Added new Sandstorm dungeon', 'Fixed mirror enemy class being incorrectly set when simulating more than once', 'Fixed pasting of copied companions'],
    'Fight Analyzer': ['Added support for Fightstring v2', 'Added option to assume maximum armor'],
    Endpoint: ['Added support for rate-limiting'],
    'All Tools': ['Added support for Necromancer', 'Added support for reworked Druid', 'Adjusted fist damage calculations for all classes']
  },
  'v7.4638': {
    Statistics: [
    ...
    'Fight Analyzer': ['Fixed simulator shortcut'],
    'Hellevator Simulator': ['Added new hellevator theme', 'Fixed prefill for characters without any hellevator progress'],
    'Guild Simulator': ['Added support for debugging tools'],
    'Pet Calendar': ['Updated pet locations'],
    'Dungeon Simulator': ['Reordered enemies in first 10 dungeons', 'Improved accuracy of Nordic Gods & Olympus dungeons', 'Added confirmation when hiding characters via statistics integration'],
    'Gold & Experience': ['Added enchantments to Quests calculations', 'Added Expeditions calculations'],
    'All Simulators': ['Fixed switch to Warrior via cheat menu', 'Fixed Assassin unarmed damage', 'Fixed custom preset dialog', 'Fixed configuration dialog growing beyond the window'],
    'All Tools': ['Added new optimized dialog system', 'Added separate translations for items and monsters', 'Added lazy loading system for separate translations', 'Added feedback form']
  },
  'v7.4528': {
    Statistics: [
    ...
    'Dungeon Simulator': ['Fixed mirror enemy health for shadow variant', 'Fixed mirror enemy class image not appearing', 'Fixed broken enemy dropdown', 'Fixed berserker dungeon enemy auto armor calculation'],
    'Guild Simulator': ['Fixed inactivity health reduction', 'Fixed displayed inactivity duration'],
    'Gold & Experience': ['Fixed Mage Tower upgrade prices'],
    Endpoint: ['Added button to select all characters in Capture selected characters mode', 'Fixed login with passwords above 40 characters', 'Fixed hidden character list not persisting across reloads']
  },
  'v7.4394': {
    Statistics: [
    ...
    'Fight Simulator': ['Removed gladiator and glove enchantments from debug presets'],
    'Hellevator Simulator': ['Added new hellevator theme', 'Fixed first round loss counting as win against all floors'],
    Endpoint: ['Updated to Unity 2023']
  },
  'v7.4306': {
    Statistics: ['Reworked Scripts & Templates', 'Added author name and description to published scripts', 'Added indicator for when new version of published scripts is found', 'Added usage count for published scripts', 'Added filter by type to script archive'],
    'Fight Analyzer': ['Added option to open prefilled player simulator for a selected group'],
    'Fight Simulator': ['Added debug option to generate custom characters'],
    'All Tools': ['Added support for Necromancer']
  },
  'v6.4154': {
    Statistics: [
    ...
    'Hellevator Simulator': ['Added support for logging', 'Added new hellevator theme'],
    'Pet Simulator': ['Fixed pet health not increasing properly when using generate', 'Fixed pet selection'],
    'Dungeon Simulator': ['Added 10th enemy of The Pyramids of Madness'],
    'All Simulators': ['Added option to select best rune automatically based on the current enemy', 'Added more debugging variables', 'Fixed shield value when morphing into warrior'],
    'All Tools': ['Added <code>SFTOOLS</code> content creator code to the main page', 'Added new link to community-made tool on the main page']
  },
  'v6.3875': {
    Statistics: [
    ...
    'All Simulators': ['Added many performance improvements', 'Added new implementation for toggle buttons', 'Replaced percentage configuration values with decimals'],
    'Hellevator Simulator': ['Added new hellevator theme'],
    Endpoint: ['Added EU4 to whitelist'],
    'All Tools': ['Added support for other websites to request character data from SFTools', 'Added new link to community-made tool on the main page']
  },
  'v6.3736': {
    Statistics: ['Added option to save table as CSV file', 'Added many performance improvements', 'Added line numbers to script editor', 'Added alternative <code>row</code> keyword for <code>show</code>', 'Fixed script editor losing focus when switching tabs', 'Removed insecure table check'],
    'Dungeon Simulator': ['Added new button for simulator threshold settings', 'Fixed debug copy'],
    'All Simulators': ['Added debug preset with maximum weapon damage ranges', 'Added new notice system'],
    Endpoint: ['Updated to newer Unity version', 'Fixed close button not clickable in specific cases', 'Fixed interruption errors not recognized properly'],
    'All Tools': ['Added new link to community-made tool on the main page', 'Fixed scrollbar not properly visible in dialogs']
  },
  'v6.3586': {
    Statistics: [
    ...
    'Pet Simulator': ['Fixed display of gladiator % damage'],
    Endpoint: ['Added support for S&F Accounts', 'Reworked communication']
  },
  'v6.3469': {
    Statistics: [
    ...
    'Dungeon Simulator': ['Fixed Twister experience display'],
    'Hellevator Simulator': ['Added theme selector', 'Added monster damage ranges'],
    'Fight Analyzer': ['Added customizable group sorting'],
    'All Simulators': ['Added reworked battlemage', 'Added reworked critical damage calculation', 'Added more accurate berserker']
  },
  'v6.3394': {
    Statistics: [
    ...
    'Fight Simulator': ['Removed debug simulator'],
    'Pet Simulator': ['Added dependency on player simulator in order to accommodate possible future reworks', 'Added support for logging'],
    'Pet Calendar': ['Reworked tool', 'Added support for translations'],
    'Fight Analyzer': [
    ...
    'All Simulators': ['Improved performance', 'Improved accuracy'],
    'All Tools': ['Fixed scrolling when a dialog is visible', 'Fixed statistics integration displaying other players when imported via integration']
  },
  'v6.3263': {
    Statistics: [
    ...
    'Fight Analyzer': ['Added new tool that serves as a replacement for Fight Viewer and Boss Viewer', 'Added missing enemy names'],
    'Hydra Simulator': ['Fixed maximum hydra level after import'],
    'Fight Simulator': ['Currently selected player will be marked as target when switching to 1 v All or All v 1 modes', 'Currently selected simulator mode will now be persistent'],
    'Fortress Simulator': ['Replaced fortifications input with a dropdown'],
    'Hellevator Simulator': ['Added prototype simulator'],
    Endpoint: ['Added proper error messages when login or data capture fails'],
    'All Tools': ['Added dark theme', 'Added new terms &amp; conditions that must be accepted before using Endpoint', 'Added links to community-made tools on the main page', 'Added updated Druid', 'Added updated Bard', 'Updated styling of all dialogs', 'Removed fight viewer tool', 'Removed boss viewer tool']
  },
  'v5.2897': {
    Statistics: [
    ...
    'Fortress Simulator': ['Redesigned simulator to allow for multiple battles to be simulated at once'],
    'Fight Simulator': ['Added option to disable attribute reduction', 'Added option to freely configure certain class behaviors', 'Redesigned tournament mode'],
    'Guild Simulator': ['Fixed gladiator being reduced'],
    'Dungeon Simulator': [
    ...
    'Pet Simulator': ['Added editable thread and iteration counts (including separate iteration count for pet maps)'],
    'Arena Manager': ['Reworked tool'],
    Endpoint: ['Added option to save first 50 characters in Hall of Fame'],
    'Gold & Experience': ['Updated gold pit capacity formula', 'Added dark theme'],
    Blacksmith: ['Added dark theme'],
    'All Simulators': ['Added progress bar to simulations', 'Added hiding of players & guilds and other options in statistics integration'],
    'All Tools': ['Updated to new version of FUI', 'Added support for Hungarian language', 'Added support for player level 701 - 800', 'Removed loading of english language file when different language is selected']
  },
  'v5.2675': {
    Statistics: [
    ...
    'Pet Simulator': ['Removed beta backend switch'],
    'Fight Simulator': ['Added option to enable fireball fix against certain classes', 'Added <code>?debug</code> parameter for simulation debugging'],
    'Arena Manager': ['Fixed runes not rounding down to 20 (as per new backend)'],
    'Dungeon Simulator': [
    ...
    'All Simulators': ['Added support for reworked Druid', 'Fixed low performance when starting All & More simulation modes', 'Fixed undefined gladiator value when using integration', 'Fixed logging for Demon Hunter and Druid'],
    'Pet Calendar': ['Fixed location descriptions'],
    'Gold & Experience': ['Added support for translations'],
    'Blacksmith Upgrades': ['Added support for translations', 'Reworked UI using new editor'],
    'Boss Viewer': ['Added option to export current data into usable sftools format'],
    'All Tools': ['Fixed content appearing only after page is fully loaded']
  },
  'v5.2469': {
    Statistics: [
    ...
    'Pet Simulator': ['Added best pets for each class to simulate all', 'Added translations'],
    'Dungeon Simulator': ['Added option to simulate all remaining enemies in a dungeon', 'Adjusted values of several dungeons'],
    'Fight Simulator': ['Added inverse of One vs. All mode', 'Set adding new players to be at the top of the player list'],
    'Guild Simulator': ['Reworked simulator'],
    'All Simulators': ['Added translations', 'Added support for drag & drop of HAR files', 'Added new class graphics', 'Fixed Bard simulation'],
    'Gold & Experience': ['Moved gold table into separate tab'],
    'Fight Viewer': ['Added support for Bard'],
    'Boss Viewer': ['Added support for character data deltas']
  },
  'v5.2250': {
    Statistics: ['Added toast message when database is being updated', 'Fixed visibility of some headers (due to new BE)', 'Fixed Bard missing instrument in player detail'],
    'Underworld Simulator': ['Added new simulator'],
    'Dungeon Simulator': ['Added option to save image of Simulate All results', 'Added class swap to cheat menu', 'Fixed armor calculation for Battlemage bosses'],
    'Fight Simulator': [
    ...
    'Pet Simulator': ['Reworked UI', 'Fixed missing input for pets between 150 - 199', 'Fixed position of New BE checkbox for small screens'],
    'Hydra Simulator': ['Fixed thirteen-headed hydra', 'Fixed poor performance when using Poll', 'Fixed drag & drop'],
    'All Simulators': [
    ...
    'All Tools': ['Removed <code>Go back</code> buttons']
  },
  'v5.2134': {
    Statistics: ['Added support for Bard class', 'Added <code>Hydra</code> field to groups', 'Reworked several dialogs', 'Fixed tracking of deleted players', 'Fixed counting of present guild members', 'Fixed crash when trying to format non-string value'],
    'Inventory Manager': ['Added support for Bard class'],
    'All Simulators': ['Added support for Bard class', 'Added support for reworked Demon Hunter class', 'Added editable block chance for warriors (please adjust your saved presets)', 'Reworked UI & simulator code'],
    'Hydra Simulator': ['Added Endpoint integration'],
    'Pet Simulator': ['Fixed Sim All button showing when no dungeons are left']
  },
  'v5.2000': {
    Statistics: [
    ...
    Endpoint: ['Added option to save friend characters'],
    'All Simulators': ['Berserkers now have chance to evade first attack'],
    'Dungeon Simulator': ['Adjusted weapon damages for berserker enemies'],
    'Fight Simulator': ['Added option to drag & drop file instead of copy & pasting', 'Fixed undefined prefix when switching to IHOF mode', "Fixed assassin's second attack missing from debug logs"],
    'Gold & Experience': ['Added fortress tab', 'Added underworld tab']
  },
  'v5.1922': {
    Statistics: [
    ...
    'Dungeon Simulator': ['Added health chart into sim-all results', 'Added missing loaders'],
    'Pet Simulator': ['Added missing loaders'],
    'Boss Viewer': ['Fixed loading of HAR files for chrome'],
    'Gold & Experience': ["Added MajkG's souls curve"]
  },
  'v5.1856': {
    Statistics: [
    ...
    'Dungeons Simulator': ['Fixed Twister quick selection'],
    'Pet Simulator': ['Changed armor toggle to include message about beta backend', 'Removed save & load feature'],
    'Gold & Experience': ["Added Enza's real gold pit values"]
  },
  'v5.1785': {
    Statistics: [
    ...
    'Dungeon Simulator': ['Available boss selection is now linked to dungeon & boss selections'],
    'Pet Simulator': ['Fixed generate not working for 20th pet']
  },
  'v5.1716': {
    Statistics: [
    ...
    'Dungeon Simulator': ['Added standalone endpoint & HAR integration', 'Added Tavern of the Dark Doppelgangers', 'Adjusted several armor values for Shadow World opponents'],
    'Pet Simulator': ['Added standalone endpoint & HAR integration', 'Added simulate all option', 'Fixed mage check for whether fight is possible'],
    'Boss Viewer': ['Added support for companion data'],
    'Inventory Manager': ['Added Mannequin', 'Fixed shop items not appearing'],
    'All tools': ['Added new Terms of Service and Changlog pop-ups', 'Added partial support for multiple languages', 'Added Sentry for error tracking', 'Added new gem look']
  },
  'v4.1443': [
  ...
  'v3.768': ['Added error text to guild tables when data is missing for any members.', 'Added option to hide files.'],
  'v3.748': [
  ...
  'v3.726': ['Reworked player detail screen.', 'Added category and header definitions. You can now create a group of options and extend any category or header with it.<br/>You can look at an example <a href="https://pastebin.com/raw/Zes71ez0">here</a>.', 'Many fixes and improvements for new expression evaluator.'],
  'v3.715': [
  ...
  'v3.669': ['You can now enable/disable lazy loading permanently in Files tab.', 'Fixed filter not working with custom expressions.', 'Fixed filter not working with custom sessions within Me tab.', 'Added info pupup to filter with all available options.'],
  'v3.649': ['Added experimental lazy loading that can be enabled by using the following url: <a href="https://sftools.mar21.eu/stats?lazy">https://sftools.mar21.eu/stats?lazy</a>'],
  'v3.630': [
  ...
  'v3.616': ['<code>Last Active</code> header no longer works as before.<br/>To use colors like before, you\'ll need to modify the header in <a href="res/update0.png">this way</a>.'],
  'v3.610': [
  ...
  'v3.606': ['Reworked dot object access.', 'Some unary operators can now be stacked infinitely.'],
  'v3.594': [
  ...
  'v3.589': ['Templates are now available also in temporary sessions.', 'Added headers for item base attributes and pet bonus percentages.', 'Added <code>round</code> and <code>fixed</code> functions for custom expressions.', 'Fixed own raidable wood and stone.'],
  'v3.575': ['Weapon damage and elemental damage runes are now shown properly for Assassins within the detail screen.', 'Added headers for weapon damage and separated elemental damage headers.'],
  'v3.559': ['Added <code>clean</code> keyword that forces header to ignore all shared options.<br/>You can look at an example <a href="https://pastebin.com/raw/WaFSAsRc">here</a>.'],
  'v3.551': ['You are now required to use <code>player</code> object to access player data.<br/>You can look at an example <a href="https://pastebin.com/raw/mZmT0hh8">here</a>.'],
  'v3.549': ['Added ability to sort columns by anything using <code>order by</code> keyword including <code>difference</code> variable to sort by difference.', 'Added way to access compared object with <code>reference</code> variable.']
}
